// ============================================
// توابع کمکی (بدون وابستگی به فایل دیگر)
// ============================================
    ...
    async function loadQuestions() {
        ...
        try {
            ...
            for (const q of questions) {
                ...
                // دکمه حذف (فقط مدیر یا صاحب سوال)
                let deleteBtn = '';
                ...
                if (q.answers && q.answers.length > 0) {
                    ...
                    answersHtml += `<p>هنوز پاسخی داده نشده است.</p>`;
                }
                ...
                // دکمه پاسخ به سوال (فقط مدیر)
                let replyBtn = '';
                if (isAdmin) {
                    replyBtn = `
                        <div style="margin-top:10px;">
                            <textarea id="reply-${q.id}" rows="2" placeholder="پاسخ خود را بنویسید..." style="width:100%; padding:8px; border-radius:12px; border:1px solid #ccc; font-family:inherit;"></textarea>
                            <button class="reply-q" data-id="${q.id}" style="margin-top:5px; background:#2ecc71; color:white; border:none; border-radius:15px; padding:6px 12px; cursor:pointer;">✅ ارسال پاسخ (مدیر)</button>
                        </div>
                    `;
                } else if (loggedUser) {
